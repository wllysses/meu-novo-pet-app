...
export function Card(props: CardProps) {
    return (
        <div className="max-w-[255px] h-[350px] w-full bg-white shadow relative">
            <span className={`absolute top-2 right-2 px-4 rounded text-white ${props.data.disponivel ? 'bg-green-500' : 'bg-red-500'}`}>{props.data.disponivel ? 'Disponível' : 'Adotado'}</span>
            <img src={props.data.imagem} alt={`Foto do pet: ${props.data.nome}`} loading="lazy" className="w-full h-[220px] object-cover"/>
